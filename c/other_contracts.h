...
/* Calculate godwoken short address of an contract account by it's corresponding ETH address
 ...
 ======
   ...
   input[32..64] => eth lock script hash

 output:
   output[12..32] => godwoken short address
 */
int eth_to_godwoken_addr_other(gw_context_t *ctx,
                         const uint8_t *code_data,
                         const size_t code_size,
                         bool is_static_call,
                         const uint8_t *input_src,
                         ...
                         uint8_t **output, size_t *output_size)
{
  ...
  memcpy(lock_script_hash, input_src + 32, 32);
  mol_seg_t new_script_seg;
  ret = build_script(lock_script_hash, 1, script_args,
                               ...
                               uint8_t **output, size_t *output_size)
{
  ...
  int ret;
  uint8_t script_args[52];
  memcpy(script_args, g_rollup_script_hash, 32);
  // memcpy(script_args + 32, (uint8_t*)(&g_creator_account_id), 4);
  // memcpy(script_args + 32 + 4, input_src + 12, 20);
  memcpy(script_args + 32, input_src + 12, 20);
  mol_seg_t new_script_seg;
  ...
  debug_print_data("script seg: ", new_script_seg.ptr, new_script_seg.size);
  *output = (uint8_t *)malloc(32);
  if (*output == NULL)
  {
    ckb_debug("malloc failed");
