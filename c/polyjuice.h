...
int load_globals(gw_context_t *ctx, uint32_t to_id, evmc_call_kind call_kind)
{
  ...
  else if (raw_args_seg.size == CONTRACT_ACCOUNT_SCRIPT_ARGS_SIZE)
  {
    ...
    if (memcmp(creator_code_hash_seg.ptr, code_hash_seg.ptr, 32) != 0 || *creator_hash_type_seg.ptr != *hash_type_seg.ptr
        /* compare rollup_script_hash */
        || memcmp(creator_raw_args_seg.ptr, raw_args_seg.ptr, 32) != 0 || creator_raw_args_seg.size != 36)
    {
                   ...
                   const evmc_message *msg_origin, struct evmc_result *res)
{
  ...
  if (res->output_size != 0 && res->output_data != NULL)
  {
    debug_print_data("[handle_message] output", res->output_data, res->output_size);
  }
